...
namespace PROProtocol
{
    public class TypesManager
    {
        ...
        {
            int[][] pokeTypes = JsonConvert.DeserializeObject<int[][]>(Properties.Resources.pokemon_types);
            Type1 = new PokemonType[pokeTypes.GetLength(0)+1];
            Type2 = new PokemonType[pokeTypes.GetLength(0)+1];
            for (int i=0; i < pokeTypes.GetLength(0); i++)
            {
                Type1[i + 1] = (PokemonType)pokeTypes[i][0];
                Type2[i + 1] = (PokemonType)pokeTypes[i][1];
            }
